...
class DfProcessor:
        ...
        try: 
            for nome_df, df in dict_df.items():
                ...
                try:
                    ...
                    df_dict_conv[nome_df] = self.converti_valori_df(df)
    ...
    def converti_valori_df(self, df: pd.DataFrame)  -> tuple[bool, pd.DataFrame | None]:
        ...
        Raises:
            ...
            ValueError: Se il DataFrame è vuoto o contiene errori di conversione critici.
            Exception: Per altri errori durante l'elaborazione.
        """
        # Validazione input
        ...
        try:
            ...
            for colonna in df_elaborato.columns:
                try:
                    print(f"Elaborazione colonna: '{colonna}'")
                    ...
                    if pd.api.types.is_object_dtype(df_elaborato[colonna].dtype):
                        try:
                            ...
                            if date_convertite is not None and date_convertite.notna().any():
                                ...
                                elif numero_valori_convertiti > 0:
                            ...
                            errori_colonne.append(f"Errore conversione date in colonna '{colonna}': {str(e)}")
                            print(f"Errore nella conversione date per colonna '{colonna}': {str(e)}")
                    ...
                    if pd.api.types.is_float_dtype(df_elaborato[colonna].dtype):
                        try:
                            def converti_float_sicuro(x):
                                if pd.notna(x):
                                    # Controlla se il valore è troppo grande per essere convertito
                                    if abs(x) > 1e15:  # Limite ragionevole per evitare overflow
                                        raise ValueError(f"Valore troppo grande per la conversione: {x}")
                                    return str(int(x)).strip()
                                return None
                            
                            df_elaborato[colonna] = df_elaborato[colonna].apply(converti_float_sicuro)
                            print(f"Colonna '{colonna}': Convertita da float a stringa.")
                            
                        except Exception as e:
                            errori_colonne.append(f"Errore conversione float->stringa in colonna '{colonna}': {str(e)}")
                            print(f"Errore nella conversione float per colonna '{colonna}': {str(e)}")
                    ...
                    if pd.api.types.is_integer_dtype(df_elaborato[colonna].dtype):
                        try:
                            ...
                            print(f"Colonna '{colonna}': Convertita da intero a stringa.")
                            
                        except Exception as e:
                            errori_colonne.append(f"Errore conversione intero->stringa in colonna '{colonna}': {str(e)}")
                            print(f"Errore nella conversione intero per colonna '{colonna}': {str(e)}")
                    ...
                    if pd.api.types.is_object_dtype(df_elaborato[colonna].dtype):
                        try:
                            def converti_stringa_sicuro(x):
                                if pd.notna(x):
                                    str_value = str(x).strip()
                                    # Controlla lunghezza ragionevole della stringa
                                    if len(str_value) > 100:  # Limite ragionevole
                                        print(f"Attenzione: stringa molto lunga nella colonna '{colonna}' truncata")
                                        str_value = str_value[:100]
                                    return str_value
                                return None
                            
                            df_elaborato[colonna] = df_elaborato[colonna].apply(converti_stringa_sicuro)
                            print(f"Colonna '{colonna}': Processata come stringa.")
                            
                        except Exception as e:
                            errori_colonne.append(f"Errore conversione stringa in colonna '{colonna}': {str(e)}")
                            print(f"Errore nella conversione stringa per colonna '{colonna}': {str(e)}")

                except Exception as e:
                    errori_colonne.append(f"Errore generale nella colonna '{colonna}': {str(e)}")
                    print(f"Errore generale nella elaborazione della colonna '{colonna}': {str(e)}")
                    continue
            
            # Verifica finale
            if len(errori_colonne) > 0:
                print(f"\nATTENZIONE: Sono stati riscontrati {len(errori_colonne)} errori durante l'elaborazione:")
                for errore in errori_colonne:
                    print(f"  - {errore}")
                return False, None
                            
            print(f"Elaborazione completata. DataFrame risultante: {df_elaborato.shape[0]} righe, {df_elaborato.shape[1]} colonne")
            return True, df_elaborato
        
        except Exception as e:
            print(f"Errore durante l'elaborazione del DataFrame: {str(e)}")
            return False, None
