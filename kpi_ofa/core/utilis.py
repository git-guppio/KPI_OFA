...
class DataFrameTools:
    ...
    def clean_data_SE16(self, data: pd.DataFrame) -> pd.DataFrame:
        """
        Verifica e pulisce i dati prima di caricarli in un DataFrame.
        ...
        try:
            ...
            for line in filtered_lines[1:]:
                ...
                if actual_pipes == expected_pipes:
                    # La riga già corretta
                    corrected_lines.append(line)                                    
                else:
                    # La riga è già corretta o ha meno pipe del previsto
                    msg = (f"La riga contiene un numero errato di separatori; attesi:{expected_pipes}, trovati:{actual_pipes}")
                    # Se la correzione non ha funzionato, segnala l'errore
                    print(msg)
                    # Genera un exception
                    raise ValueError(msg) 
    ...
    # ------------------------------------------------------------------------------------------
    #  Metodi per la normalizzazione dei nomi delle colonne del dataframe delle estrazioni SAP
    # ------------------------------------------------------------------------------------------

    def normalize_and_concat(self, dataframes_dict, transaction_code, use_friendly_names=False, strict_mode=True):
        """
        ...
        """
    # -- Funzione interna per normalizzare i nomi delle colonne
